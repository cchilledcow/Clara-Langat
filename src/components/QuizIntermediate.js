...
import "./Quiz.css";
import quizDataIntermediate from "../api/QuizDataIntermediate";
...
function QuizIntermediate() {
  ...
  const handleNextBtn = () => {
    if (displayQuestion < quizDataIntermediate.questions.length - 1) {
      setDisplayQuestion(displayQuestion + 1);
      setDisplayAnswer(false);
      setAnswerCorrect(false);
      setClickable(true);
      ...
      setShowResults(true);
      setResult((amountCorrect / quizDataIntermediate.questions.length) * 100);
    }
  };

  const handleAnswer = (index) => {
    if (quizDataIntermediate.questions[displayQuestion].correctAnswerIndex == index) {
      ...
      {!showResult ? (
        <React.Fragment>
          <h2>
            ...
            {quizDataIntermediate.questions.length}){" "}
          </h2>

          <div className="quiz-options">
            <h2 className="question">
              {" "}
              {quizDataIntermediate.questions[displayQuestion].question}
            </h2>

            {quizDataIntermediate.questions[displayQuestion].answers.map(
              (answer, index) => {
                return (
                  <button
                    key={index}
                  ...
                  >
                    <h3
                      style={
                        displayAnswer &&
                        quizDataIntermediate.questions[displayQuestion]
